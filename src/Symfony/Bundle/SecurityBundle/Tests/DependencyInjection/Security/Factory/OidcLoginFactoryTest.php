...
class OidcLoginFactoryTest extends TestCase
{
    ...
    public function testRejectsAPublicClientWithoutPkce()
    {
        ...
        $this->expectException(InvalidConfigurationException::class);
        $this->expectExceptionMessage('cannot be false when "token_endpoint_auth_method" is "none"');
    ...
    public function testAConfidentialClientMayDisablePkce()
    {
        // PKCE is only mandatory for a public client: a confidential client keeps the
        // option, e.g. for a provider that does not support PKCE yet
        $factory = new OidcLoginFactory();
